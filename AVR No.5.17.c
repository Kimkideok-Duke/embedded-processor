...
#define FND_Null        0           //초기값이 0으로 표시되기 위해 null값을 0으로 수정
#define FND_Star        14
#define FND_Sharp       15
...
unsigned int buf_seg[5] = {FND_Null,FND_Null,FND_Null,FND_Null,FND_Null}; 
bit Key_off_flag=0;
...
unsigned char keypad_in()               // 채터링 방지 구문 
{
      ...
      if(buf_seg[4])                         //buf_seg[4]에 값이 입력될때
      {                                      //buf_seg[1]~buf_seg[3]이 0으로 초기화
            buf_seg[0] = 0;
            buf_seg[1] = 0;  
            buf_seg[2] = 0;
            buf_seg[3] = 0;
